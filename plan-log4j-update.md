# Log4j Migration Plan: mqtt-client

**Layer**: 4 — update after `jre-utils` and `serialization`.
...
| `OWN_NEW_VERSION` | What version should `mqtt-client` be bumped to? (currently `1.0.0`) |
...
- **Artifact**: `info.unterrainer.commons:mqtt-client`
...
Increment `<version>` (line 18, currently `1.0.0`).
...
| File | Action |
|------|--------|
| `pom.xml` | Update parent version, update jre-utils + serialization versions, bump own version |
